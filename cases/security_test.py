...
from pandora.host_pb2 import Connection, OwnAddressType, DataTypes
...
class SecurityTest(base_test.BaseTestClass):  # type: ignore[misc]
    ...
    @avatar.asynchronous
    async def test_ssp_ctkd(
        self,
        connect: Union[Literal['outgoing_connection'], Literal['incoming_connection']],
        pair: Union[Literal['outgoing_pairing'], Literal['incoming_pairing']],
        variant: Union[
            Literal['accept'],
            Literal['rejected_in_smp'],
            Literal['timeout_in_smp'],
            Literal['disconnect_in_smp'],
            Literal['disconnected_in_smp'],
        ],
        ref_io_capability: Union[
            Literal['against_default_io_cap'],
            Literal['against_no_output_no_input'],
        ...
        if isinstance(self.ref, BumblePandoraDevice):
            ...
            self.ref.server_config.io_capability = io_capability
            # TODO: Map and set key distribution
        ...
        try:
            ...
            if not connect_and_pair_task.done():
                ...
                if dut_ev.method_variant() in ('numeric_comparison', 'just_works'):
                    ...
                    dut_ev_answer = PairingEventAnswer(event=dut_ev, confirm=confirm)
                    ref_ev_answer = PairingEventAnswer(event=ref_ev, confirm=confirm)
        ...
        # SMP Stage.
        try:
            # On Bumble side - not very sure. It seems we have removed auto CTKD on Bumble?
            # TODO: In BumblePandoraServer, there will be a second PairingDelegate.accept() call here
            # Pending the accept, so that we can perform reject/disconnect/timeout.
            assert dut_ref and ref_dut
            if variant == 'disconnect_in_smp':
                await self.dut.aio.host.Disconnect(connection=dut_ref)
            elif variant == 'disconnected_in_smp':
                await self.ref.aio.host.Disconnect(connection=ref_dut)
            elif variant == 'rejected_in_smp':
                # await self.ref.reject...
                pass
            elif variant == 'timeout_in_smp':
                await asyncio.sleep(40.0)
            else:
                # await self.ref.accept...
                pass
        finally:

            # TODO:
            #   1. On bumble side, check key existance
            #   2. Make LE connection using public address.
            if variant == 'accept':
                # Should pass
                pass
            else:
                # Should fail
                # TODO: All bond may be lost on Android?
                pass
